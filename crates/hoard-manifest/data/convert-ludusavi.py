...
The catalog is now generated by the Rust converter (`ludusavi::convert_yaml`,
driven by the `regenerate_embedded_catalog` test — see ../data/README.md), so
that what ships and what an in-app refresh produces cannot drift apart. This
script predates `install_dirs`, `launch_exes`, `steam_extra_ids`,
`lutris_slug` and `cloud_steam`, writes uncompressed JSON under the old
filename, and does not emit the title index at all: running it would quietly
strip process matching and every `<base>` template from detection.
